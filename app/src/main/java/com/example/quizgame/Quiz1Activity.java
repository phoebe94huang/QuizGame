...
public class Quiz1Activity extends AppCompatActivity {
    ...
    private void checkAnswer() {
        timer.cancel();
        if(answer4.isChecked()) { // Correct answer
            score += 1; // Add 1
            updateScore(score);
            Toast.makeText(Quiz1Activity.this, "That's correct! Great job! +1", Toast.LENGTH_SHORT).show();
            Intent intent = new Intent(Quiz1Activity.this, Quiz2Activity.class); // Goes to next question
            intent.putExtra("score", score); // pass amount earned
            startActivity(intent);
            finish();
        } else { // Wrong answer
            Toast.makeText(Quiz1Activity.this, "That's incorrect. The correct answer is: Aaron Judge", Toast.LENGTH_SHORT).show();
